...
{ 
    public class Stock
    {
...

        {
            try
            {
                string qry = @"INSERT INTO   PURCHASE.TRANS_HD (VSLCODE, TRANS_NO, CTRANS_NO, TRANS_TYPE_CODE, TRANS_DATE, TRANS_FROM, UPDFLAG, DE_BY, DE_AT)
                               VALUES   ('" + stock.vessel_code + "','" + stock.trans_no +"','" + stock.ctrans_no + "','" + stock.trans_type_code + "','" + stock.trans_date + "','" + stock.trans_from + "','" + stock.updflag + "','" + stock.data_entered_by + "','" + stock.data_entered_date + "')";
                Update_Insert UI = new Update_Insert();
                int result = UI.OperationsOnSourcecDB(qry);
...

        {
            try
            {
                string qry = @"INSERT INTO   PURCHASE.TRANS_DT (VSLCODE, TRANS_NO, IM_CODE, CODE_TYPE, TRANS_QTY, UPDFLAG, PLAN_QTY, DE_BY, DE_AT)
                               VALUES   ('" + stock.vessel_code + "','" + stock.trans_no + "','" + stock.item_code + "','" + stock.code_type + "','" + stock.trans_qty + "','" + stock.updflag + "','" + stock.plan_qty+"','" + stock.data_entered_by + "','" + stock.data_entered_date + "')";

...
namespace sqlBase
{
    class WorksDone
    {
...

        {
            try
            {
                //Selecting values for calculations 
                ...
                else if (tbl.Rows[0]["FQ_TYPE"].ToString() == "H")
                {
                    ...
                    object last_done_hrs = sq.ExecuteScalarOnSourceDB("Select RH_PRESENT FROM PURCHASE.EQ_MF WHERE EQ_CODE = " + WorksDone.eq_code);
                    string qry = @"UPDATE PMS.JOB_PLAN  SET        NEXT_DUE_DATE = '" + res +
                                                              "', LAST_DONE_DATE = '" + WorksDone.jo_end_date +
                                                                "', NEXT_DUE_HRS = '" + next_due_hrs +
                                                               "', LAST_DONE_HRS = '" + last_done_hrs +
                                                                     "', UPDFLAG = 'C'" +
                                                               "' WHERE  JP_CODE = '" + WorksDone.jp_code + "'";
                    DBOperations UI = new DBOperations();
                    int result = UI.OperationsOnSourceDB(qry);
...

        {
            try
            {
                //Getting values for calculation of rob_qty and total_out
...

        {
            try
            {
                // Getting values for trans_no calculation
...

        {
            try
            {
                ...
                //Calculation for getting trans_type_code and dept_code
...

        {
            try
            {
                //Getting values for inserting to job_order table
